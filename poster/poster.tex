...
\begin{document}
  ...
  \begin{columns}[onlytextwidth]%
    \begin{column}{0.55\textwidth}%
      ...
      \textbf{Comparison of PROPOSAL implementation to calculations from NIST Standard Reference Database}

      \vspace{0.5cm}

      \begin{tikzpicture}
        \centering
        \node [anchor=west, align=left] (noteA) at (-7.5,4) {Expected discrepancies \\around absorption lines\\ (here: K-line of \textup{Ar})};
        \node [anchor=west, align=left] (noteB) at (-7.5, 12) {Agreement better than \\\SI{10}{\percent} even at low \\energies where used \\parametrization is not \\expected to be exact };        
        \node [anchor=west, align=left] (noteC) at (+27, 8) {Good agreement \\for energies where\\ pair production and\\ Compton are \\dominant processes};        
